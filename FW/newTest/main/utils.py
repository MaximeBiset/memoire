...
def discriminate_demandobjs(request, demands):
    if request.user.is_superuser:
        messages.add_message(request, messages.INFO, _('Vous êtes administrateur, vous pouvez voir toutes les demandes d\'objets.'))
        return demands
    if demands.first() and is_branch_admin(request.user, demands.first().branch):
        messages.add_message(request, messages.INFO, _('Vous êtes administrateur de la branche, vous pouvez voir toutes les demandes d\'objets.'))
        return demands
